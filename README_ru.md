...
### 🧪 Каналы обновлений: Stable и Lab

У ClashFX два канала обновлений. Выбирайте любой — оба полноценные.

| | 🟢 Stable (стабильный) | 🟠 Lab (экспериментальный) |
|---|---|---|
| **По умолчанию** | ✅ | — |
| **Частота релизов** | Раз в 2–7 дней, когда накопятся исправления | Сразу после исправления — иногда ежедневно |
| **Содержимое** | Только проверенные исправления | Свежие исправления + иногда экспериментальные функции |
| **Визуальная метка** | Нет | Маленькая оранжевая точка в правом верхнем углу иконки в Dock / Программах |

**Переключиться на Lab**: откройте `ClashFX → Настройки → Отладка → Канал обновлений → Лаборатория (экспериментальный)` и подтвердите. Оранжевая точка появится сразу. Вернуться на Stable можно из того же места в любой момент.

**Нашли баг в Lab?** Откройте `Справка → Отправить отзыв…` — GitHub Issue с очищенным диагностическим отчётом будет предзаполнен. Или `Справка → Скопировать диагностику…`, чтобы скопировать отчёт вручную.

Пользователи Stable: вы уже на рекомендуемом канале — ничего делать не нужно.
